...
from dataset.large.EEG.EEG_generate import generate_EEG_data
from dataset.large.abalone.abalone_generate import generate_abalone_data
from dataset.large.avlia.avila_generate import generate_avlia_data
from dataset.large.letter.letter_generate import generate_letter_data
from dataset.large.waveform.waveform_generate import generate_waveform_data
...
def experiemnt_adp(data, real_labels,alpha,l,theta):
    ARI_record=[]
    ...
    while (True):
        a = uncertainty_selection(predict_labels, data, neighbors, result_dict,nearest_neighbors)
        if a == None:
            print("查询完毕")
            break
        else:
        ...
        neighbors, count = dynamic_selection(real_labels, candidates, neighbors, x, count)
        predict_labels, result_dict = neighbors_labeling(ascription_tree, neighbors)
        iter=iter+1
...
if __name__ == '__main__':
    ...
    # "letter"
    data, real_labels = generate_letter_data("../../dataset/large/letter/letter-recognition.data")
    ARI_record=experiemnt_adp(data, real_labels, alpha, l, theta)
