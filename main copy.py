...
def refine_by_h_5_cputime(ET: nx.Graph, roots: list, data, real_labels, subroots: list, dataName: str = 'nan', alpha=.5, **kwargs):
    '''
    1. 计算边的度数
    '''
    # print(f'start refine_by_h 5')
    ...
    for edge, cc in sorted_edge_neighbors_count:
        ...
        if c >= n:
            # print(f'return at {i} iteration, where c==n')
            break
    return
...
def run_cpu_time_(data, real_labels, n_centers, tht):
    # data = pdata.copy(deep=True)
    time_start = time.process_time()
    ET, roots, subroots = run_PRSC(data, real_labels, n_centers, tht)
    # neighbors = NearestNeighbors(n_neighbors=2).fit(data)
    # distance, nearest_neighbors = neighbors.kneighbors(
    #     data, return_distance=True)
    # distance = distance[:, 1]
    ...
    refine_by_h_5_cputime(
        ET, roots, data, real_labels, subroots, dataName='', alpha=.5)
...
def repetition_runp_h(rdata, real_labels, k, theta, i, cup_time, dataName) -> None:
    # rdata, real_labels = run_CPU_time_on_synthetic_data(size, k=k)
    # rdata = pd.DataFrame(rdata)
    # data = rdata.copy()
    pdata = (rdata - rdata.mean()) / (rdata.std())
    # avtime = 0
    # rt = 2
    # for tht in range(0, rt):
    #     data = pdata.copy(deep=True)
    #     time_start = time.process_time()
    #     ET, roots, subroots = run_PRSC(data, real_labels, n_centers, tht)
    #     # neighbors = NearestNeighbors(n_neighbors=2).fit(data)
    ...
    #         ET, roots, data, real_labels, subroots, dataName='', alpha=.5)
    #     time_end = time.process_time()
    #     avtime += time_end - time_start
    # cup_time['time'][i] = avtime/rt

    ###############################

    result = Parallel(n_jobs=4, batch_size=2)(delayed(run_cpu_time_)
                                              (pdata.copy(deep=True), real_labels, k, tht=theta) for i in range(8))
    cup_time['time'][i] = np.mean(result)

    os.makedirs('result/exp-cpu/theta', exist_ok=True)
    pd.DataFrame(cup_time).to_csv(
        f'result/exp-cpu/theta/{dataName}.csv', index=False)


def runp_CPU_time(datapath=None) -> None:
    warnings.filterwarnings("ignore")
    # assert os.path.exists(dataDir)
    # 生成聚类数据
    dataName = datapath.split(".")[0]
    step = [500] + [i*1000 for i in range(1, 11)]
    cup_time = {'datasetSize': step, 'time': [0]*len(step)}
    step = np.arange(.05, .51, .05)
    cup_time = {'theta': step, 'time': [0]*len(step)}
    dataDir = 'data'
    k = 3
    rdata, real_labels, k, _ = DataLoader.get_data_from_local(
        f'{dataDir}/{datapath}', doPerturb=True, frac=1)

    jobs = len(step)
    n_jobs: int = 4
    batch_size = max(1, (jobs + n_jobs-1) // n_jobs)

    Parallel(n_jobs=n_jobs, batch_size=batch_size)(delayed(repetition_runp_h)(
        rdata, real_labels, k, theta, i, cup_time, dataName) for i, theta in enumerate(cup_time['theta']))

    # for i, theta in enumerate(cup_time['theta']):
    #     # rdata, real_labels = run_CPU_time_on_synthetic_data(size, k=k)
...
def runp_p(dataDir, file: str, outdir: str = 'result/p2/small', tht=1) -> None:
    ...
    # run_alg_turn_p(data, real_labels, K, tht)
...
def exp_prameter():
    ...
    thata = [i for i in range(1, 11)]
    alpha = [i/10 for i in range(11)]
    alpha = np.arange(0, 1.1, .2)
    df = {"dataName": [], "theta": [], "alpha": [], "AUIC": []}
    dataN = {'iris.csv': 150, 'thyroid.csv': 215, 'ionosphere.csv': 351,
             'musk.csv': 476, 'led.csv': 500, 'banknote.csv': 1347,
             'Segmentation.csv': 2310, 'OptDigits.csv': 5620, 'Statlog.csv': 6435,
             'Pen-Based Digits.csv': 10992, 'Online Shoppers.csv': 12058,
             'Dry Bean.csv': 13543, 'Letter Recognition.csv': 20000, 'Avila.csv': 20867}
    # 定义 dashes 和 markers 样式
    ...
    markers_styles = ['o', 's', 'D', '^', 'v', '<',
                      '>', 'p', '*', 'h', 'H', '+', 'x', '1']

    # 使用 itertools.cycle 来循环使用这些样式
    dashes_cycle = itertools.cycle(dashes_styles)
    markers_cycle = itertools.cycle(markers_styles)
    ...
    files = ['iris.csv', 'thyroid.csv', 'ionosphere.csv',
             ...
             'Pen-Based Digits.csv', 'Online Shoppers.csv',
             'Dry Bean.csv', 'Letter Recognition.csv', 'Avila.csv']
    # files = ['iris.csv', 'thyroid.csv',
    #          'musk.csv',  'banknote.csv',
    #          'Segmentation.csv', 'OptDigits.csv', 'Statlog.csv',
    #          'Pen-Based Digits.csv', 'Online Shoppers.csv',
    #          'Dry Bean.csv', 'Letter Recognition.csv', 'Avila.csv']
    ...
    for t in thata:
        for a in alpha:
            fs = os.listdir(f'result/exp-p/theta-{t*100}/alpha-{a:.1f}')
            for f in fs:
                if dataN.get(f, 0) > 1000:
                    data = pd.read_csv(
                        f'result/exp-p/theta-{t*100}/alpha-{a:.1f}/{f}')
                    df['dataName'].append(f.split('.')[0])
                    df['theta'].append(t)
                    df['alpha'].append(a)
                    df["AUIC"].append(calculate_AUIC(data, n=dataN[f]))

    df = pd.DataFrame(df)
    df['theta'] = df['theta'].apply(lambda x: x*100)
    # 删除dataName为iris.csv的行
    # droplist = ['iris', 'led', 'ionosphere']
    # for d in droplist:
    #     df = df[df['dataName'] != d]
    # sort DataFrame by theta and alpha
    df = df.sort_values(by=['theta', 'alpha'])
    df.to_csv('result/exp-p/info.csv', index=False)
    ...
    all_vars = files.copy()
    all_vars.reverse()
    # all_vars = list(set([var for vars in subplots_vars for var in vars]))
    color_palette = sns.color_palette("husl", n_colors=len(all_vars))
    color_map = dict(zip(all_vars, color_palette))

    dfg = df.groupby('theta')
    # plt.figure(figsize=(12, 8))
    fig, axes = plt.subplots(2, 5, figsize=(16, 20))
    ...
    for name, group in dfg:
        group = group.pivot(
            index='alpha', columns='dataName', values='AUIC')
        col = group.columns
        # print(f'{col=}')

        for c in group.columns:
            style = plot_styles[c]
            sns.lineplot(x='alpha', data=group, y=c, linestyle=style['dashes'], marker=style['marker'],
                         ax=axes[i], color=color_map[c], legend=False)
        axes[i].set_title(f'$\\theta={name}$')
        axes[i].set_xlabel('$\\alpha$')
        axes[i].set_ylabel('AUIC@n')
    ...
    handles, labels = [], []
    for var in all_vars:
        style = plot_styles[var]
        line = plt.Line2D([0], [0], color=color_map[var], lw=2,
                          linestyle=style['dashes'], marker=style['marker'])
        handles.append(line)
        labels.append(var)

    # 将图例添加到整个图的下面,并且排列为一行
    plt.legend(handles, labels, loc='lower left',
               bbox_to_anchor=(-4.9, -.2), ncol=14,
               fontsize=14)
    # plt.legend(handles, labels, loc='center ', bbox_to_anchor=(1, 0.5))
    plt.tight_layout()
    plt.show()
    ...
    for name, group in dfg:
        group = group.pivot(
            index='theta', columns='dataName', values='AUIC')
        for c in group.columns:
            style = plot_styles[c]
            sns.lineplot(x='theta', data=group, y=c, linestyle=style['dashes'], marker=style['marker'],
                         ax=axes[i], color=color_map[c], legend=False, linewidth=4)
        fontsize = 24
        axes[i].set_title(f'$\\alpha={name:.1f}$', fontsize=fontsize)
        axes[i].set_xticks(np.arange(200, 1001, 200))
        axes[i].set_xlim(100, 1000)
        axes[i].set_xlabel('$\\theta$', fontsize=fontsize)
        axes[i].set_ylabel('AUICAUIC@n', fontsize=fontsize)
...
if __name__ == '__main__':
    dataN = {'iris.csv': 150, 'thyroid.csv': 215, 'ionosphere.csv': 351,
             ...
             'Pen-Based Digits.csv': 10992, 'Online Shoppers.csv': 12058,
             'Dry Bean.csv': 13543, 'Letter Recognition.csv': 20000, 'Avila.csv': 20867,
             'HTRU2.csv': 17898, 'adult.csv': 48842, 'codon.csv': 13032}
    dataN = {'iris.csv': 150, 'thyroid.csv': 215, 'ionosphere.csv': 351,
             'musk.csv': 476, 'led.csv': 500, 'banknote.csv': 1347,
             'Segmentation.csv': 2310, 'OptDigits.csv': 5620, 'Statlog.csv': 6435,
             'Pen-Based Digits.csv': 10992, 'Online Shoppers.csv': 12058,
             'Dry Bean.csv': 13543, 'Letter Recognition.csv': 20000, 'Avila.csv': 20867,
             'HTRU2.csv': 17898, 'adult.csv': 48842, 'codon.csv': 13032}
    # for data in dataN.keys():
    # runp_CPU_time(data)
    run()
    jobs = len(dataN.keys())
    n_jobs: int = 2
    batch_size = max(1, (jobs + n_jobs-1) // n_jobs)
    Parallel(n_jobs=n_jobs, batch_size=batch_size)(delayed(runp_CPU_time)
                                                   (data)for data in dataN.keys())
    # exp_prameter()
    # run()
