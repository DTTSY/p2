...
import member_generation.subspace as sm
...
def _get_constraints_file_names(i,dataset_name, additional_postfix, postfixes=None):
    """
    ...
    return map(lambda x: _default_constraints_folder +'L'+str(i)+'/'+ dataset_name + '_' + x + additional_postfix + '.txt',
               use_postfixes)
...
def _get_file_name(dataset_name, n_cluster_lower_bound, n_cluster_upper_bound,
    ...
    constraint_file_suffix = '' if not is_constraint_method else ('_' + constraint_file.split('Constraints/L'+str(1)+'/')[1])
...
def generate_libs_by_constraints(dataset_name,
    ...
    constraints_files = _get_constraints_file_names(1,dataset_name, additional_postfix, postfixes=postfixes)
...
#    res_names = []
    for constraints_file in constraints_files:
        t2 = generate_library(data, target, dataset_name, n_members, class_num,
                                    n_cluster_lower_bound=n_cluster_lower_bound,
                                    n_cluster_upper_bound=n_cluster_upper_bound,
...
#    t2=time.clock()
    if generate_only or is_constrained:
        print("saveing lib+++++++at",savepath + filename + '_pure' + '.res')
