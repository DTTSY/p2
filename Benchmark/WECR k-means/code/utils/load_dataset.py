...
def drift_load():
    ...
    for i in  range(1,11,1):       
        fr = open('./UCI Data/drift/batch'+str(i)+'.dat')
        for line in fr.readlines():
#            print(line)
            cur_line=re.split(" |:|;", line.strip())
            
            
#            cur_line.remove(cur_line[1])
#            print(cur_line)
            target.append(int(cur_line[0])-1)
##            cur_line.remove(cur_line[0])
            flt_line = cur_line[1:-1]
...
#            print(np.array(dataset))
            datasets = np.concatenate((datasets,np.array(dataset)), axis=0)
            targets = np.concatenate((targets,np.array(target)), axis=0)
#            
        dataset = []
        target = []
#    
...
#    print(np.array(targets))
    ...
    sio.savemat("UCI Data/drift/drift_image.mat", {"foo":np.array(datasets)})
    sio.savemat("UCI Data/drift/drift_labels.mat",{"foo":np.array(targets)})
    return np.array(datasets), np.array(targets)

    
def unpickle_cifar():
    import pickle
    with open("UCI Data/cifar/cifar-10-python/cifar-10-batches-py/test_batch", 'rb') as fo:
...
def mnist_reader( ):
    ...
    labels_path = "./UCI Data/fashion_mnist/t10k-labels-idx1-ubyte.gz"

    images_path = "./UCI Data/fashion_mnist/t10k-images-idx3-ubyte.gz"
...
def load_segmentation(normalized=False):
    target = []
    dataset = []
...
def load_coil20():
    data = sio.loadmat('../data/UCI Data/COIL20/COIL20.mat')
    fea = data['fea']
...
def load_SMK_CAN_187():
    data = sio.loadmat('UCI Data/SMK_CAN_187/SMK_CAN_187.mat')
    fea = data['X']
    fea = fea.astype(np.float64)
    labels = data['Y'] - 1
    labels = labels.flatten()
...
def load_GLI_85():
    data = sio.loadmat('UCI Data/GLI_85/GLI_85.mat')
    fea = data['X']
    fea = fea.astype(np.float64)
    labels = data['Y'] - 1
    labels = labels.flatten()
...
def load_Allaml():
    data = sio.loadmat('UCI Data/Allaml/Allaml.mat')
    fea = data['X']
    fea = fea.astype(np.float64)
    labels = data['Y'] - 1
    labels = labels.flatten()
...
def load_TOX():
    data = sio.loadmat('UCI Data/TOX/TOX.mat')
    fea = data['X']
    fea = fea.astype(np.float64)
    labels = data['Y'] - 1
    labels = labels.flatten()
...
#    X_train, y_train = mnist_reader.load_mnist('data/fashion', kind='train')
    X_test, y_test = mnist_reader( )
#    sio.savemat("UCI Data/fm/fashion_minst_image.mat", {"foo":X_test})
#    sio.savemat("UCI Data/fm/fashion_minst_labels.mat",{"foo":y_test})
    return X_test, y_test

def load_semeion(normalized=True):

    fr = open('UCI Data/semeion/semeion.data')
    count = 0
    feature_vectors = []
    target = []
    for line in fr.readlines():
