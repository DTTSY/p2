...
from ensemble import weighted_ensemble as we
from evaluation import internal_metrics as ie
import numpy as np
import time
...
def consistency_per_cluster_efficient(label, mlset, nlset, cons_type='both'):
    ...
    for i in range(0, n_clusters):
        ...
        if cons_type == 'must':
            ...
            cluster_constraints_i = count_CL[i] 
            cluster_satisfy = satisfy_CL[i]
            weight_i = float(cluster_satisfy) / cluster_constraints_i if cluster_constraints_i > 0 else 1
            # gamma_i = _get_g_gamma(len(label[label == i]), cluster_constraints_i, n_ml + n_cl, n_instances, gamma=gamma)
            consistencies[i] = weight_i
            cluster_constraints[i] = cluster_constraints_i
...
def constrain_score(library_name,lable,mlset, nlset,g_gama):
#    cons_type = ['both', 'must']
    cons_type='both'
    
#    con_per_cluster, cluster_cons_num =consistency_per_cluster_efficient(lable, mlset, nlset, cons_type=cons_type)
   
    con_clustering=consistency(lable, mlset, nlset, cons_type=cons_type)
#    
#    dataset_name = library_name.split('_')[0]  
#    d, t = exp_data.dataset[dataset_name]['data']()
#
##    internals=[]
##    internals=(ie.new_cal_internal_weights_for_library_cluster_as_array(d,lable))
##    internals.append(ie._silhouette(d, lable))
#    
#
#
#    con_per_cluster = []
...
#
#
#    weight, cluster_cons_num = consistency_per_cluster_efficient(lable, mlset, nlset, cons_type=cons_type)
#    con_per_cluster.append(weight)
#    constraints_num.append(cluster_cons_num)
#
#    con_clustering.append(consistency(lable, mlset, nlset, cons_type=cons_type))

        
#    print("*************weight************************")
#    print(lable)
#    print(con_clustering[0])
#    print(con_per_cluster[0])
#    print(g_gama[0])
#    print(type(ce.new_create_weighted_membership_matrix_extended(lable,con_clustering[0],con_per_cluster[0], g_gama[0])))
#    return (np.sum(ce.new_create_weighted_membership_matrix_extended(lable,con_clustering[0],con_per_cluster[0], g_gama[0])))
    return con_clustering