...
def comparison_methods(i,dataset_name, constraints_files=None, additional_postfix='', eval_method=None):
    """
    ...
    :param eval_method:
    """
    filename = _default_eval_path +'L'+str(i)+'/'+ dataset_name + '_' + time.strftime('%Y-%m-%d_%H_%M_%S',
                                                                       time.localtime(time.time())) + '.csv'
    with open(filename, 'w') as f:
        ...
        for filename in filenames:
            ml, cl = io_func.read_constraints(_default_constraints_folder +'L'+str(i)+'/'+ filename + '.txt')
            for method in eval_methods:
