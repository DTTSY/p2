...
#    print(cluster_runs[0])
#    print(clustering_weights[0])
#    print(cluster_level_weights[0])
#    print(alpha[0])
#    print(internal[0])
...
#    calculate weighted hypergraph_adjacency 
        ...
        hypergraph_adjacency = build_weighted_hypergraph_adjacency_extended(cluster_runs,
                                                                            ...
                                                                            alpha,
                                                                            internal)
#        print("************hypergraph_adjacency_bulid*************")
#        print(hypergraph_adjacency)
    else:
        if alpha is None:
            alpha = 1
...
#      maybe  weight matrix
    ...
    s = scipy.sparse.csr_matrix.dot(hypergraph_adjacency.transpose().tocsr(),
                                    scipy.sparse.csr_matrix(([1]*hypergraph_adjacency.data.size,
                                                            ...
                                                            hypergraph_adjacency.indptr),shape=hypergraph_adjacency.shape))
#    print("********************s***********************************")  
#
#    print(s)                                                      
...
#            print( cluster_weights)
            ...
            print(np.ones(np.where(cluster_run == elt)[0].size, dtype=np.float32))
...
def new_create_weighted_membership_matrix_extended(cluster_run, clustering_weight, cluster_weights, alpha):
    """
    weighted version of 'create_membership_matrix'
    ...
    if reduce(operator.mul, cluster_run.shape, 1) != max(cluster_run.shape):
        ...
        for elt in cluster_ids:
            indices = np.append(indices, np.where(cluster_run == elt)[0])
            indptr = np.append(indptr, indices.size)
            weight = ((1 - alpha[int(elt)]) * clustering_weight + alpha[int(elt)] * cluster_weights[int(elt)]) 
            # cluster-level weighting
            cluster_data = np.ones(np.where(cluster_run == elt)[0].size, dtype=np.float32) * weight
            data = np.append(data, cluster_data)
